...
/** @typedef {"energy"|"item"|"fluid"|"gas"|"overclock"} NetworkType */

const NETWORK_TYPES = new Set(["energy", "item", "fluid", "gas", "overclock"]);
const NETWORK_TAGS = Object.freeze({
  energy: "dorios:energy",
  item: "dorios:item",
  fluid: "dorios:fluid",
  gas: "dorios:gas",
  overclock: "dorios:overclock_network",
});
...
/**
 * Updates several logical networks while reading the center and six neighbors
 * only once. Universal geometry is refreshed once per affected block.
 *
 * @param {Block} changedBlock
...
export function updateNetworksAtMany(changedBlock, types) {
  ...
  for (const type of touchedTypes) {
    ...
    else if (type === "item") scheduleItemNetworkRescan(changedBlock.location, dimension);
  }
...
world.afterEvents.playerBreakBlock.subscribe(({ block, brokenBlockPermutation }) => {
  ...
  system.run(() => {
    ...
    if (brokenBlockPermutation.hasTag("dorios:overclock_network")) types.push("overclock");
    ...
    if (updateNetworksAtMany(block, /** @type {NetworkType[]} */ (types)).has("item")) {
...
world.afterEvents.playerPlaceBlock.subscribe(({ block }) => {
  ...
  system.run(() => {
    ...
    if (placedBlock.hasTag("dorios:overclock_network")) types.push("overclock");
    ...
    if (updateNetworksAtMany(placedBlock, /** @type {NetworkType[]} */ (types)).has("item")) {
      invalidateItemContainerAt(dimension, location);
    }
  });
});
...
world.afterEvents.pistonActivate.subscribe(({ piston, isExpanding, dimension }) => {
  ...
  system.runTimeout(() => {
    ...
    for (const { target: location, source: pairedLocation } of movements) {
      ...
      if (block.hasTag("dorios:overclock_network") || pairedBlock.hasTag("dorios:overclock_network")) {
        types.push("overclock");
      }

      // Containers do not necessarily carry dorios:item, so always let the
      // adjacent item nodes decide whether a rebuild is needed.
      const firstTouchesItemNetwork = updateNetworksAtMany(
        block,
        /** @type {NetworkType[]} */ (types),
      ).has("item");
      const secondTouchesItemNetwork = updateNetworksAtMany(
        pairedBlock,
        /** @type {NetworkType[]} */ (types),
      ).has("item");
