...
function dropBonsaiItem(dimension, itemId, pos) {
  if (typeof itemId !== "string" || itemId === "empty") return;

  try {
    dimension.spawnItem(new ItemStack(itemId, 1), pos);
  } catch (err) {
    console.warn(`[UtilityCraft] Failed to drop bonsai item "${itemId}": ${err}`);
  }
}
...
DoriosAPI.register.blockComponent("bonsai", {
  ...
  onPlayerInteract({ player, block }) {
    ...
    if (itemId.includes("hoe") || itemId.includes("aiot")) {
      ...
      const unbreakingLvl = enchantable.hasEnchantment("unbreaking") ? enchantable.getEnchantment("unbreaking").level : 0;
    ...
    if (bonsai) {
      const soilId = block.getState("utilitycraft:soil");
      const isValidSoil = specialSoils.includes(soilId) || bonsai.allowed.includes(soilId.split(":")[1]);
  ...
  onPlayerBreak({ brokenBlockPermutation, block }) {
    ...
    const entityPos = { x: x + 0.5, y: y + 0.172, z: z + 0.5 };

    const soil = brokenBlockPermutation.getState("utilitycraft:soil");
    dropBonsaiItem(block.dimension, soil, pos);

    if (brokenBlockPermutation.getState("utilitycraft:hasBonsai")) {
      const entity = block.dimension.getEntities({ tags: ["bonsai"], maxDistance: 0.1, location: entityPos })[0];
      if (entity) {
        const bonsaiEntity = bonsaiItems.find((enty) => enty.entity === entity.typeId);
        dropBonsaiItem(block.dimension, bonsaiEntity?.sapling, pos);
        entity.addTag("despawn");
      }
    }
  },
...
system.afterEvents.scriptEventReceive.subscribe((event) => {
  ...
  drops.forEach((loot) => {
    if (Math.random() <= loot.chance) {
      let amount = Array.isArray(loot.amount) ? DoriosAPI.math.randomInterval(loot.amount[0], loot.amount[1]) : loot.amount;
