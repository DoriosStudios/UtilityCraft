...
const empty = PNG.sync.read(fs.readFileSync(path.join(root, 'RP/textures/ui/heavy_water/f_00.png')));
if (empty.width !== 48 || empty.height !== 48) throw new Error('Expected a 48x48 empty bar');
const reference = PNG.sync.read(fs.readFileSync(path.join(root, 'RP/textures/ui/heavy_water/f_48.png')));
...
for (const type of ['crude_oil', 'petroleum', 'diesel']) {
  ...
  const dir = path.join(root, `RP/textures/ui/${type}`);
  ...
  for (let frame = 0; frame <= 48; frame++) {
    ...
    fs.writeFileSync(path.join(dir, `f_${String(frame).padStart(2, '0')}.png`), PNG.sync.write(out));
  }
